...
from src.exceptions import Submitted
from src.utils.serialize import recursive_merge
...
class DockerEnvironment:
    def __init__(
        ...
        **kwargs,
        ...
        """
        self.logger = logger or logging.getLogger("jacc.environment")
    ...
    def _start_container(self):
        ...
        container_name = f"jacc-{uuid.uuid4().hex[:8]}"
        cmd = [
    ...
    def cleanup(self):
        ...
        if container_id is None:  # if init fails early, container_id might not be set
            return
        for args in (
            [self.config.executable, "stop", container_id],
            ...
            try:
                ...
                self.logger.debug("Docker cleanup command failed: %s", shlex.join(args), exc_info=True)
