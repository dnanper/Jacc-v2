...
def format_edge(graph: KnowledgeGraph, source_id: str, rel_type: str, target_id: str) -> str:
...
def _print_summary(graph: KnowledgeGraph, paths: list[str], sample_size: int, edge_sample_size: int) -> None:
    ...
    if edge_sample_size > 0:
        ...
        for rel in graph.relationships[:edge_sample_size]:
            print(f"  {format_edge(graph, rel.source_id, str(rel.type), rel.target_id)}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run the repo_explorer structure ingestion pipeline in memory.")
